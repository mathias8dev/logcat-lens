...
test('creates experimental artifact versions and display names', () => {
	...
	assert.equal(packageDisplayNameFor('Logcat Universal', 'experimental'), 'Logcat Universal Experimental');
});

test('keeps release artifact versions and display names unchanged', () => {
	assert.equal(artifactVersionFor('3.1.0', 'release'), '3.1.0');
	assert.equal(packageDisplayNameFor('Logcat Universal', 'release'), 'Logcat Universal');
});
...
test('parses package options', () => {
	...
	assert.deepEqual(packageOptions(['experimental', '--tag', '--out', '/tmp/logcat-universal.vsix']), {
		mode: 'experimental',
		createTag: true,
		help: false,
		out: '/tmp/logcat-universal.vsix',
	});
});
