...
…LogView Universal will recognise the bytes by their magic header and show the preview.

**Limitations** (these are device-side, not viewer-side):

- Android's `Log.d` truncates a single record at ~4 KB. For media bodies larger than that, the app must split the base64 into ~3.5 KB chunks before logging — LogView Universal reassembles the chunks automatically. A single oversized `Log.d` call loses its tail on the device.
...
- For serious HTTP body inspection, pair LogView Universal with an in-app inspector like Chucker or Axer. LogView Universal is best for *seeing* media inline with the rest of your logs, not as a replacement for a dedicated HTTP inspector.
...
1. Open the **LogView Universal** tab in the bottom panel
...
Found a bug or have a feature request? [Open an issue](https://github.com/mathias8dev/logview-universal/issues).

Want to contribute? [Submit a pull request](https://github.com/mathias8dev/logview-universal/pulls) — all contributions are welcome!

## License

LogView Universal is licensed under the [MIT License](LICENSE).
